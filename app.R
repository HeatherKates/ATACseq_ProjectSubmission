...
ui <- fluidPage(
  useShinyjs(), 
  titlePanel("ATAC-seq Project Submission"),
    mainPanel(
      tabsetPanel(
        id = "main_tabs",
        tabPanel("Project Metadata",
                 ...
                 textInput("seqID", "Sequencing ID (SeqID)", placeholder = "e.g. NS3982"),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit_button, {
    ...
    output_dir <- file.path("data_submissions", folder_id)
    ...
    # Copy the submission folder to the public directory
    dir.create(public_dir, recursive = TRUE)
    file.copy(from = list.files(output_dir, full.names = TRUE), 
              to = public_dir, 
              overwrite = TRUE)
    
    # Construct public URL
    public_url <- glue::glue("https://data.rc.ufl.edu/pub/cancercenter-dept/BCB-SR/project_submissions/{folder_id}/")
    
    # Show confirmation modal with public URL
    showModal(modalDialog(
      title = "Submission Saved",
      HTML(glue::glue("A record of this submission is available at:<br><a href='{public_url}' target='_blank'>{public_url}</a>")),
      easyClose = TRUE
    ))
